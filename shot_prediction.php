...
<?php
    ...
    if ($d_line != 'All' && $d_length != 'All') {
        $query = $con->query(
            "SELECT Shot_type, SUM(Count) AS count
            FROM line_length_deliverytype_shot_runs_innings_aovers
            WHERE Delivery_Length <> 'Delivery_Length' AND Delivery_Length <> 'Null' AND Delivery_Line <> 'Null' 
            AND Shot_type <> 'Null' AND Shot_type <> 'Unknown' AND Delivery_Length <> 'Unknown' AND Delivery_Line <> 'Unknown' 
    ...
    elseif($d_line != 'All'){
        $query = $con->query(
            ...
            AND Shot_type <> 'Null' AND Shot_type <> 'Unknown'
            AND OverNumber BETWEEN '$over_no_low' AND '$over_no_high'
...
//     SELECT Shot_type, 
//        SUM(Count) AS count, 
//        (SUM(Count) / (SELECT SUM(Count) 
//                       FROM line_length_deliverytype_shot_runs_innings_aovers 
//                       WHERE Delivery_Length <> 'Delivery_Length' 
//                         AND Shot_type <> 'Null' 
//                         AND Shot_type <> 'Unknown' 
//                         AND OverNumber BETWEEN '$over_no_low' AND '$over_no_high')
//        ) * 100 AS percentage
// FROM line_length_deliverytype_shot_runs_innings_aovers
// WHERE Delivery_Length <> 'Delivery_Length' 
...
<h2>Probable Shot Types</h2>

<form action="shot_prediction.php" method="post">
    ...
    <option value="Shortter">Shortter</option>
    </select>
    ...
    <option value="MiddleOrLegStump">Middle Or Leg Stump</option>
    ...
    <option value="OutsideOffStump">Outside Off Stump</option>
    <option value="Straight">Straight</option>
    <option value="Stumps">Stumps</option>
    <option value="Wide">Wide</option>
    <option value="WideOff">Wide Off</option>
    <option value="WideOutsideOff">Wide Outside Off</option>
    <option value="DownTheLegSide">Down The Leg Side</option>
    <option value="MiddleStumps">Middle Stumps</option>
    <option value="OffStumps">Off Stumps</option>
...
<script>
  ...
  const labels = <?php echo json_encode($Shot_type); ?>;
  const data = {
  labels: labels,
  datasets: [{
