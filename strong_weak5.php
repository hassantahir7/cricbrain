...
<?php
    ...
    if ($d_line != 'All' && $d_length != 'All') {
        ...
        $query = $con->query(
            "SELECT Shot_type, SUM(Runs) AS runs
            FROM line_length_deliverytype_shot_runs_innings_aovers
            WHERE Delivery_Length <> 'Delivery_Length' AND Shot_type <> 'Null' 
            AND Shot_type <> 'Unknown'
            AND OverNumber BETWEEN '$over_no_low' AND '$over_no_high'
            AND Delivery_Line = '{$d_line}' AND Delivery_Length = '{$d_length}'
            GROUP BY Shot_type
            HAVING runs > 2
            ");
    }
...
<h2>Most Efficient Shot Type</h2>

<form action="strong_weak5.php" method="post">
					...
					<div class="row">
						...
						<div class="col-6">
							<button onclick="showNext()"
								style="padding-left: 30px; padding-right: 30px; margin-left: 265px; background-color: #00235a; border-radius: 5px; border: none;">
								<a href="strong_weak6.php" style="text-decoration: none;">
...
<script>
  ...
  labels: labels,
  datasets: [{
    label: 'Delivery Length - Delivery Line VS Runs',
