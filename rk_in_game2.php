...
<?php
      ...
      switch ($overs) {
        ...
        $query = $con->query(
            "SELECT CONCAT(Shot_type) AS xAxis, SUM(Count) AS count
            FROM rk_length_line_shot_aovers 
            WHERE Delivery_Length <> 'Null' AND Delivery_Length <> 'Delivery_Length' AND Delivery_Line <> 'Null' AND Shot_type <> 'Null'
            AND Delivery_Length <> 'Unknown' AND Delivery_Line <> 'Unknown' AND Shot_type <> 'Unknown'
        ...
        $query = $con->query(
            "SELECT CONCAT(Delivery_Length, ' - ', Shot_type) AS xAxis, SUM(Count) AS count
            FROM rk_length_line_shot_aovers 
            WHERE Delivery_Length <> 'Null' AND Delivery_Length <> 'Delivery_Length' AND Delivery_Line <> 'Null' AND Shot_type <> 'Null'
            AND Delivery_Length <> 'Unknown' AND Delivery_Line <> 'Unknown' AND Shot_type <> 'Unknown'
            AND Over_No BETWEEN '$over_no_low' AND '$over_no_high'
        ...
        $query = $con->query(
            "SELECT CONCAT(Delivery_Line, ' - ', Shot_type) AS xAxis, SUM(Count) AS count
            FROM rk_length_line_shot_aovers 
            WHERE Delivery_Length <> 'Null' AND Delivery_Length <> 'Delivery_Length' AND Delivery_Line <> 'Null' AND Shot_type <> 'Null'
            AND Delivery_Length <> 'Unknown' AND Delivery_Line <> 'Unknown' AND Shot_type <> 'Unknown'
            AND Over_No BETWEEN '$over_no_low' AND '$over_no_high'
            ...
            $query = $con->query(
            "SELECT CONCAT(Delivery_Length,' - ', Delivery_Line, ' - ', Shot_type) AS xAxis, SUM(Count) AS count
            FROM rk_length_line_shot_aovers 
            WHERE Delivery_Length <> 'Null' AND Delivery_Length <> 'Delivery_Length' AND Delivery_Line <> 'Null' AND Shot_type <> 'Null'
            AND Delivery_Length <> 'Unknown' AND Delivery_Line <> 'Unknown' AND Shot_type <> 'Unknown'
            GROUP BY xAxis
...
<h2><strong>Most Played Shot Type</strong></h2>

<form action="rk_in_game2.php" method="post">
...
const data = {
    labels: labels,
  datasets: [{
    label: 'Delivery Length -  Delivery Line - Shot Type VS Count',
