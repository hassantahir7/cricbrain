...
<body>
  ...
  include ('_connection.php');
            ...
            $query = $con->query(
                ...
                FROM rk_line_runs_aovers 
                WHERE Delivery_Line <> 'Null' AND Delivery_Line <> 'Delivery_Line' AND Runs <> -1
                AND Delivery_Line <> 'Unknown' AND OverNumber BETWEEN 1 AND 6
                GROUP BY Delivery_Line
                HAVING runs > 1
                ");
            ...
            elseif($_GET['range'] == '7_15'){
                ...
                $query = $con->query(
                    ...
                    FROM rk_line_runs_aovers 
                    WHERE Delivery_Line <> 'Null' AND Delivery_Line <> 'Delivery_Line' AND Runs <> -1
                    AND Delivery_Line <> 'Unknown' AND OverNumber BETWEEN 7 AND 15
                    GROUP BY Delivery_Line
                    HAVING runs > 1
                    ");
            ...
            elseif($_GET['range'] == '16_20'){
                ...
                $query = $con->query(
                    ...
                    FROM rk_line_runs_aovers 
                    WHERE Delivery_Line <> 'Null' AND Delivery_Line <> 'Delivery_Line' AND Runs <> -1
                    AND Delivery_Line <> 'Unknown' AND OverNumber BETWEEN 16 AND 20
                    GROUP BY Delivery_Line
                    HAVING runs > 1
                    ");
            ...
            {
              $Delivery_Line[] = $data['Delivery_Line'];
              $Runs[] = $data['runs'];
            }
?>
    ...
    <div class="m-2 float-lg-right">
      <nav>
        <div class="nav nav-tabs" id="nav-tab" role="tablist">
          <a href="rk_strong_weak4.php?range=all" class="nav-item nav-link <?= (empty($_GET['range']) || $_GET['range'] == 'all') ? 'active' : '' ?>" id="nav-1w-tab" role="tab" aria-controls="nav-1w" title="all">All</a>
          <a href="rk_strong_weak4.php?range=1_6" class="nav-item nav-link <?= $_GET['range'] == '1_6' ? 'active' : '' ?>" id="nav-1m-tab" role="tab" aria-controls="nav-1m" title="1_6">1-6</a>
          <a href="rk_strong_weak4.php?range=7_15" class="nav-item nav-link <?= $_GET['range'] == '7_15' ? 'active' : '' ?>" id="nav-6m-tab" role="tab" aria-controls="nav-6m" title="7_15">7-15</a>
          <a href="rk_strong_weak4.php?range=16_20" class="nav-item nav-link <?= $_GET['range'] == '16_20' ? 'active' : '' ?>" id="nav-1y-tab" role="tab" aria-controls="nav-1y" title="16_20">16-20 Overs</a>
        </div>
      </nav>
    </div>
  </div>
</div>
<div style="margin-left: 120px;"><h2><strong>Delivery Line</strong></h2></div>
